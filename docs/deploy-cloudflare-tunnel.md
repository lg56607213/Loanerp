...
### CODEF 시크릿 — 기능은 걷어냈지만 노출은 남아 있다

2026-09, CODEF 계좌연동을 쓰지 않기로 하고 관련 코드·설정·화면을 모두 제거했다.
**그래도 시크릿 노출은 해소되지 않는다.** 평문 값이 이미 푸시된 커밋 `0fb8417` 에
남아 있고 저장소가 공개이기 때문이다. 코드에서 지운 것과 히스토리에서 사라지는 것은 다르다.

쓰지 않을 기능이므로 가장 확실한 조치는 **CODEF 콘솔에서 해당 자격증명을 폐기**하는 것이다.
재발급받아 보관할 이유가 없다.

- CODEF 콘솔에서 client-secret 폐기 (재발급 불필요 — 더 이상 쓰지 않는다)
