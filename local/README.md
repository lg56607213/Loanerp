...
> **상시 가동할 서버 PC를 구축하고 외부(집·카페)에서 접속하려면**
> [`docs/server-pc-setup.md`](../docs/server-pc-setup.md) 를 먼저 읽어라.
> 이 문서의 절차를 다른 PC에서 끝까지 해보고 막혔던 지점들을 정리해 두었다.
...
- MySQL 3306은 **차단되어 있다고 단정하지 말 것.** 배포 문서 §7에 차단해 두었다고 적혀 있었으나
  실제로 확인해보니 `Profile=Any, RemoteAddress=Any`로 열려 있었다. 직접 확인하고 막는다
  (`docs/server-pc-setup.md` §5-1). 로컬 접속은 영향 없다.
